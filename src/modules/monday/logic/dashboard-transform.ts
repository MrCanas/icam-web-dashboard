import type { MondayColumn, MondayItem } from "@/modules/monday/data/types";
import type {
  MondayAsset,
  MondayFunnelMetric,
  MondayKpiBundle,
...
export function normalizeStage(raw: string | null): MondayStage {
  const key = toKey(raw ?? "");
...
export function buildMondayAssets(items: MondayItem[], columns: MondayColumn[]): MondayAsset[] {
  ...
  return items.map((item) => {
    ...
    return {
      ...
      boardGroupId: item.group?.id ?? null,
      boardGroupTitle: item.group?.title ?? null,
